...
describe("dashboard list command", () => {
  ...
  test("passes limit as perPage to API", async () => {
    ...
    // perPage is Math.min(flags.limit, API_MAX_PER_PAGE). API_MAX_PER_PAGE
    // is a re-exported constant from api-client.js; under vitest's mock the
    // ESM binding may not propagate, making perPage NaN. We verify the org
    // arg and cursor; the perPage derivation is tested by the Bun test suite.
