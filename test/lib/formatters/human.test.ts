...
describe("formatUpgradeResult", () => {
  ...
  test("renders changelog with What's new heading and indented entries", () => {
    withPlain(() => {
      const result = formatUpgradeResult({
        action: "upgraded",
      ...
      // "What's new" heading with emoji
      expect(result).toContain("What's new");
      // Category headings
      expect(result).toContain("New Features");
      expect(result).toContain("Bug Fixes");
      // Entries present
      expect(result).toContain("Add dashboard charts");
      expect(result).toContain("Fix table rendering");
      // Indented (lines start with spaces)
      const changelogStart = result.indexOf("What's new");
      const afterChangelog = result.slice(changelogStart);
      const entryLine = afterChangelog
        .split("\n")
        .find((l: string) => l.includes("Add dashboard charts"));
  ...
  test("does not render changelog when undefined", () => {
    withPlain(() => {
      const result = formatUpgradeResult({
        action: "upgraded",
        currentVersion: "0.5.0",
  ...
  test("does not render changelog when sections are empty", () => {
    withPlain(() => {
      const result = formatUpgradeResult({
        action: "upgraded",
        currentVersion: "0.5.0",
  ...
  test("hides sections with whitespace-only markdown", () => {
    withPlain(() => {
      const result = formatUpgradeResult({
        action: "upgraded",
        currentVersion: "0.5.0",
      ...
      // Should show fixes but not features
      expect(result).toContain("Bug Fixes");
      expect(result).toContain("Real fix");
      expect(result).not.toContain("New Features");
    });
  });

  test("shows truncation message when changelog is truncated", () => {
    withPlain(() => {
      const result = formatUpgradeResult({
        action: "upgraded",
        currentVersion: "0.5.0",
        ...
        changelog: {
          fromVersion: "0.5.0",
          toVersion: "0.6.0",
          sections: [{ category: "features", markdown: "- Feature one\n" }],
          totalItems: 1,
          truncated: true,
          originalCount: 10,
        },
      });
      expect(result).toContain("...and 9 more changes");
      expect(result).toContain("github.com/getsentry/cli/releases");
    });
  });

  test("returns empty changelog when all sections are whitespace", () => {
    withPlain(() => {
      const result = formatUpgradeResult({
        action: "upgraded",
        currentVersion: "0.5.0",
  ...
  test("clamps changelog to terminal height when output would overflow", () => {
    // Simulate a tiny terminal to trigger the line-clamping branch.
    const origRows = process.stdout.rows;
    try {
      // Set terminal to 10 rows → maxLines = max(5, floor(10*1.3) - 6) = 7
      ...
      withPlain(() => {
        // Generate many list items to exceed 7 rendered lines
        ...
        const result = formatUpgradeResult({
          ...
          changelog: {
            fromVersion: "0.5.0",
            toVersion: "0.6.0",
            sections: [{ category: "features", markdown: `${manyItems}\n` }],
            totalItems: 20,
            truncated: false,
            originalCount: 20,
          },
        });
        // Should be clamped — shows truncation indicator
        expect(result).toContain("...and more");
        expect(result).toContain("github.com/getsentry/cli/releases");
        // Should NOT contain all 20 items
        expect(result).not.toContain("Feature number 20");
      });
    } finally {
