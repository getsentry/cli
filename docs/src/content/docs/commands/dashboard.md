...
| `-s, --sort <sort>` | Order by (prefix - for desc). Use --sort="-count" (with =) to avoid flag alias conflicts |
| `-n, --limit <limit>` | Result limit. Required when using --group-by. Table widgets cap at 10 rows |
