...
The install script accepts these flags and environment variables:
...
| `SENTRY_VERSION` | Same as `--version` (flag takes precedence) |
| `SENTRY_INSTALL_DIR` | Override the install directory (default: `~/.local/bin` or `~/.sentry/bin`) |
| `SENTRY_INIT=1` | Run `sentry init` after installing to set up your project |
