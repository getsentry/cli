...
type InitFlags = {
  ...
  /**
   * Boolean opt-in for the OpenTUI full-screen interface. Stricli
   * auto-generates a negated `--no-tui` flag for the user-facing
   * escape hatch — both forms feed the same boolean value here.
   */
  ...
  parameters: {
    ...
    flags: {
      ...
      tui: {
        kind: "boolean",
        brief:
          "Use the experimental full-screen OpenTUI interface (Bun binary only). Pass --no-tui to force the legacy single-line interface.",
        default: false,
      },
    },
    aliases: {
  ...
  async *func(
    ...
    try {
      await runWizard({
        ...
        // `--no-tui` is auto-generated by stricli's flag negation: it
        // sets `flags.tui` to `false` (vs. the default of `false` when
        // unspecified). To distinguish "user wants legacy" from "user
        // didn't pass --tui", we treat `tui === true` as opt-in and
        // leave `forceLegacyUi` for env-var / programmatic callers.
        tui: flags.tui,
      });
    } finally {
