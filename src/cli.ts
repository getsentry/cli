  ...
  const scopeRecoveryMiddleware: ErrorMiddleware = async (next, argv) => {
    try {
      ...
      const confirmed = await logModule
        .withTag("auth")
        .prompt(
          `Missing scope(s): ${scopeList}. Re-authenticate to add them?`,
      ...
      process.stderr.write("\n");
      const { resolveOAuthScopeString } = await import("./lib/oauth.js");
      const scope = resolveOAuthScopeString({ scopes });
      ...
      // Login failed or was cancelled — fall through to the original error.
      process.exitCode = 1;
    }
  };
