...
const workflow = readFileSync(new URL('../workflows/pr-risk-jev.yml', import.meta.url), 'utf8');
const script = workflow.match(/^          script: \|\n((?:            .*\n|\n)+)/m)?.[1];
assert.ok(script, 'Publish risk label script must exist');
const AsyncFunction = Object.getPrototypeOf(async function () {}).constructor;
const publish = new AsyncFunction('require', 'github', 'context', 'core', script);
...
async function withPR(options, check) {
  ...
  const repositoryLabels = new Set(options.repositoryLabels ?? ['risk: low', 'risk: medium', 'risk: high']);
  const writes = [];
  const warnings = [];
  const pr = {
    state: 'open', head: { sha: head }, base: { sha: base },
    title: 'Update behavior', body: 'Describe this change', ...options.pr,
  };
  const result = {
    repo: 'example/cli', number: 42, status: 'ok', risk_label: 'low',
    snapshot: { source_head_sha: head, source_base_sha: base }, ...options.result,
  };
  const error = status => Object.assign(new Error(`GitHub HTTP ${status}`), { status });
  const github = { rest: {
    pulls: { get: async () => ({ data: structuredClone({ ...pr, labels: [...labels].map(name => ({ name })) }) }) },
    issues: {
      getLabel: async ({ name }) => {
  ...
  try {
    ...
    if (!options.missingResult) writeFileSync('risk-pr-result.json', JSON.stringify(result));
    await check({
      run: () => publish(require, github, { repo }, { warning: message => warnings.push(message), info: () => {} }),
      labels, writes, warnings,
    });
  } finally {
...
for (const risk of ['low', 'medium', 'high']) {
  test(`publishes ${risk}, replacing only risk labels`, async () => {
    await withPR({ result: { risk_label: risk }, labels: ['area: auth', 'risk: low', 'risk: medium', 'risk: high'] }, async ({ run, labels }) => {
      await run();
...
test('context rejection removes stale risk labels and remains unclassified', async () => {
  await withPR({ result: { status: 'context_rejected', risk_label: null } }, async ({ run, labels, warnings }) => {
    await run();
...
test('moved or closed PRs are left untouched before publication', async t => {
  for (const pr of [{ head: { sha: 'c'.repeat(40) } }, { base: { sha: 'd'.repeat(40) } }, { state: 'closed' }]) {
...
test('movement or metadata edits during publication roll back the verdict', async t => {
  for (const [name, afterAdd] of [
    ['head', pr => { pr.head.sha = 'c'.repeat(40); }],
    ['title', pr => { pr.title = 'A different change'; }],
...
test('results for another PR or an invalid risk cannot publish', async t => {
  for (const result of [{ repo: 'example/another' }, { number: 43 }, { risk_label: 'critical' }, { risk_label: 'toString' }]) {
...
test('concurrent repository label creation still publishes', async () => {
  await withPR({ repositoryLabels: [], createRace: true }, async ({ run, labels }) => {
    await run();
...
for (const risk of ['low', 'medium', 'high']) {
  test(`unchanged ${risk} leaves the PR timeline untouched`, async () => {
    await withPR({ result: { risk_label: risk }, labels: ['area: auth', `risk: ${risk}`] }, async ({ run, labels, writes }) => {
      await run();
      assert.deepEqual(writes, []);
...
test('an existing verdict removes competing risk labels without re-adding itself', async () => {
  await withPR({ result: { risk_label: 'medium' }, labels: ['area: auth', 'risk: medium', 'risk: high'] }, async ({ run, labels, writes }) => {
    await run();
