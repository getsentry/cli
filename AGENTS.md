...
* **Pagination contextKey must include all query-varying parameters with escaping**: Pagination \`contextKey\` must encode every query-varying parameter (sort, query, period) with \`escapeContextKeyValue()\` (replaces \`|\` with \`%7C\`). Always provide a fallback before escaping since \`flags.period\` may be \`undefined\` in tests despite having a default: \`flags.period ? escapeContextKeyValue(flags.period) : "90d"\`. Note: \`DEFAULT\_PERIOD = "90d"\` is defined as a module constant in commands that need it (e.g., \`issue/list.ts\`). When comparing periods, use \`formatTimeRangeFlag(flags.period) !== DEFAULT\_PERIOD\` or the \`appendPeriodHint()\` helper from \`time-range.ts\`.
...
* **Stricli parse functions can perform validation and sanitization at flag-parse time**: Stricli's \`parse\` function on \`kind: "parsed"\` flags runs during argument parsing before \`func()\` executes. It can throw errors (including \`ValidationError\`) and log warnings. Current uses: \`parseCursorFlag\`, \`sanitizeQuery\` (OR-rewrite), \`parsePeriod\` (returns \`TimeRange\`), \`parseSort\`/\`parseSortFlag\`, \`numberParser\`/\`parseLimit\`. All widespread flags use parse-time validation. For optional period flags, \`flags.period\` is \`TimeRange | undefined\` — commands use \`TIME\_RANGE\_\*\` constants as defaults. \`formatTimeRangeFlag()\` converts \`TimeRange\` back to display strings. \`appendPeriodHint(parts, period, flagName)\` in \`time-range.ts\` encapsulates the repeated \`if (formatted !== DEFAULT\_PERIOD) parts.push(...)\` pattern used across 4+ hint builders (issue/trace/span/event list). Avoids double-calling \`formatTimeRangeFlag\`.
