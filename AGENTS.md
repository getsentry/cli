...
<!-- lore:019db25e-332e-7bde-8956-25fe90965e82 -->
* **esbuild doesn't support \`with { type: "text" }\` — needs a plugin**: esbuild errors on Bun's \`import x from "./f.js" with { type: "text" }\` attribute ("Importing with a type attribute of \\"text\\" is not supported"). Our build pipeline is esbuild → bin.js → Bun compile, so esbuild sees the import first. Fix: \`script/text-import-plugin.ts\` — tiny esbuild plugin that intercepts imports with \`pluginData.with?.type === "text"\` (via \`onResolve\`/\`onLoad\`), reads the file, and emits \`export default "\<contents>"\`. Shared between \`script/build.ts\` (CLI binary) and \`script/bundle.ts\` (npm library). Bun runtime (dev, \`bun test\`) handles the attribute natively, so no plugin needed there. TypeScript also doesn't model the attribute — cast \`import x from "./f.js" with {...}\` through \`as unknown as string\`.
