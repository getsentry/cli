...
* **Biome lint differs between local lint:fix and CI lint**: Biome lint differs between local \`lint:fix\` and CI \`lint\`. Auto-fix may hide issues CI still catches: (1) \`noPrecisionLoss\` on integer literals >2^53, (2) \`noIncrementDecrement\` on \`count++\`, (3) import ordering when a named import follows non-import runtime code. Formatter rewrites multi-line imports to single-line when they fit — auto-fix may leave stale state. Always run \`bun run lint\` before pushing. Use \`for...of\` with destructuring or explicit \`i += 1\` instead of \`++\`. Use \`Number(string)\` or split literals instead of \`1\_735\_689\_600\_000\_000\_001\`.
...
* **Identity-scoped response cache via fingerprint mixin**: Identity-scoped response cache: key is \`hmac-sha256(identity, method|url)\` with module-constant HMAC key (quiets CodeQL weak-hash warning). Identity from \`getIdentityFingerprint()\` in \`db/auth.ts\`: \`oauth:\<hash(refresh\_token)>\` (stable across hourly access-token refresh), \`env:\<hash(raw\_token)>\`, or \`ANON\_IDENTITY\`. \`CacheEntry\` persists \`identity\` so \`invalidateCachedResponsesMatching(prefix)\` can skip entries owned by other identities (threaded via \`WriteRequest\` bundle to stay under \`useMaxParams=4\`). Filename hash alone isn't enough for prefix sweeps — only for exact-URL invalidation. \`clearAuth()\` dynamically imports \`clearResponseCache\` to break db/auth.ts ↔ response-cache.ts cycle. Batch mutation invalidation (e.g., \`mergeIssues\` with N IDs): split into per-ID detail-only helpers + single post-loop list sweep (\`invalidateIssueDetailCaches\` + \`invalidateOrgIssueList\` in \`api/issues.ts\`) — a combined per-ID call does N+1 full directory scans.
