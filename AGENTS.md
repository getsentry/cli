...
* **Sentry issue list --query passes OR/AND operators to API causing 400**: Fixed in PR #727. \`sanitizeQuery()\` in \`issue/list.ts\` now tokenizes \`--query\` respecting quoted strings before checking for operators. AND is auto-stripped with \`log.warn()\` (same semantics as space-separation). OR throws \`ValidationError\` with alternatives. Uses token-based detection (\`token === 'AND'\`) instead of regex, avoiding false matches in qualifier values like \`tag:OR\` or quoted strings like \`message:"error AND timeout"\`. The function sits between \`--limit\` validation and \`parsePeriod()\` in \`func()\`.
...
<!-- lore:019d8741-f639-7770-b455-8ae65a82b261 -->
* **Fuzzy project slug auto-recovery across three resolution sites**: \`tryFuzzyProjectRecovery()\` in \`resolve-target.ts\` provides cross-org fuzzy project slug recovery. Called at three sites when \`findProjectsBySlug()\` returns empty: \`resolveProjectBySlug()\` (line ~1209), \`resolveOrgProjectTarget()\` project-search case (line ~1383), and \`handleProjectSearch()\` in \`org-list.ts\` (line ~749). Uses \`findSimilarProjectsAcrossOrgs()\` which lists projects per org with p-limit concurrency, deduplicates slugs before \`fuzzyMatch()\`, then returns org-qualified matches. Auto-recovers on single strong match with \`log.warn()\`; shows suggestions for multiple/weak matches. \`handleProjectSearch\` has a \`\_isRecoveryAttempt\` guard to prevent infinite recursion on re-entry.
...
* **Query sanitization uses tokenization to respect quoted strings**: \`sanitizeQuery()\` in \`issue/list.ts\` tokenizes the query respecting quoted strings before checking for boolean operators. Tokenizer regex: \`/\S+:"\[^"]\*"|"\[^"]\*"|\S+/g\` — keeps \`message:"error AND timeout"\` as one token. Only bare \`AND\`/\`OR\` tokens (exact string match) are treated as operators. AND is auto-stripped with \`log.warn()\` (identical semantics to space-separation). OR throws \`ValidationError\` (genuinely different semantics). Exported via \`\_\_testing\` for unit tests. Key edge cases: qualifier values like \`tag:OR\`, quoted strings, leading AND.
