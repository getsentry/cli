...
* **Skill files generation pipeline and staleness checks**: \`generate-skill.ts\` walks the Stricli route tree to produce \`SKILL.md\` + per-group \`references/\*.md\` + \`index.json\` under \`plugins/sentry-cli/skills/sentry-cli/\`, AND generates \`src/generated/skill-content.ts\` — a TypeScript module that inlines all skill file contents as a \`ReadonlyMap\<string, string>\`. This module is imported by \`agent-skills.ts\` to write files to disk without any network fetching. The generator must write a stub \`skill-content.ts\` before dynamically importing \`src/app.ts\` (chicken-and-egg: app.ts transitively imports agent-skills.ts which imports skill-content.ts). Uses \`await import()\` not static \`import\` for the route tree. \`generate:skill\` is chained before \`build\`, \`dev\`, \`typecheck\`, and \`test\` in package.json, like \`generate:sdk\`. \`check-skill.ts\` validates both skill files AND \`skill-content.ts\` for staleness.
