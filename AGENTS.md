...
* **Sentry search in-list syntax constraints from PEG grammar**: Search query parsing now uses a pre-compiled PEG grammar (\`src/lib/search-query.pegjs\`, ~120 lines) instead of regex-based tokenization. Generated parser lives at \`src/generated/search-parser.js\` (~31KB), built by \`script/generate-parser.ts\` via Peggy. Grammar classifies nodes as: \`text\_filter\`, \`comparison\_filter\` (with operator), \`text\_in\_filter\` (bracket syntax), \`boolean\_op\`, \`free\_text\`, \`paren\_group\`. Key grammar gotcha: \`free\_text\` must match whole words (\`\[^\s():]+\` not char-by-char) to prevent "error" being split at "or" substring. \`is:\`/\`has:\` detected post-parse in TypeScript (not grammar). Paren groups are opaque nodes — OR merging cannot cross them. The \`generate:parser\` script runs before \`generate:sdk\` in all build chains.
...
* **Sentry issue list --query passes OR/AND operators to API causing 400**: Sentry search does NOT support AND/OR (\`allow\_boolean=False\`). \`sanitizeQuery()\` in \`src/lib/search-query.ts\` now uses a PEG parser \[\[019d917d-cf52-7e98-b3db-b25979d445f2]] instead of regex tokenization. AND stripped transparently. OR attempts rewrite: \`key:val1 OR key:val2\` → \`key:\[val1,val2]\`. Merging blocked for: \`is:\`/\`has:\` keys, negated qualifiers, comparison filters (detected structurally by parser as \`comparison\_filter\` nodes — no regex reject-list needed), wildcards, free-text tokens, different keys across OR boundary, and paren groups (opaque, can't merge across). Successful rewrites warn via \`logger.warn()\`. Failed rewrites throw \`ValidationError\`.
