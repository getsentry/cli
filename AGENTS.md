...
* **Sentry trace-logs API is org-scoped, not project-scoped**: The Sentry trace-logs endpoint (\`/organizations/{org}/trace-logs/\`) only requires org context, not project. It queries all projects in the org for logs associated with a trace ID. This differs from \`trace view\` which uses \`resolveOrgAndProject()\`. The \`trace logs\` command should use \`resolveOrg()\` instead. Params: \`traceId\`, \`statsPeriod\` (default 14d), \`per\_page\`, \`query\`, \`sort: -timestamp\`.
...
* **Sentry CLI markdown-first formatting pipeline replaces ad-hoc ANSI**: Formatters build CommonMark strings; \`renderMarkdown()\` renders to ANSI for TTY or raw markdown for non-TTY. Key helpers: \`colorTag("red", text)\` for semantic colors, \`mdKvTable()\` for detail views, \`mdRow()\` for streaming rows, \`mdTableHeader()\` for headers (\`:\` suffix = right-aligned), \`renderTextTable()\` for batch tables with box-drawing. \`isPlainOutput()\` checks \`SENTRY\_PLAIN\_OUTPUT\` > \`NO\_COLOR\` > \`!isTTY\`. Custom marked renderer replaced marked-terminal, eliminating ~970KB deps. \`cli-highlight\` used directly for code blocks.
...
* **abbreviateCount toFixed(1) rounding at tier boundaries**: In \`abbreviateCount\`, \`toFixed(1)\` can round past thresholds (99.95 → "100.0K") and \`Math.round\` can produce ≥1000 ("1000K"). Fixes: pre-compute \`Number(scaled.toFixed(1))\` for comparisons; promote to next tier when \`rounded >= 1000\`; cap at 999 at max tier. NaN returns \`"?".padStart(COL\_COUNT)\` + \`Sentry.logger.warn()\`. Use \`raw.length <= COL\_COUNT\` instead of hardcoded thresholds.
...
<!-- lore:019ca9c3-989f-72b6-aaa8-eeba944585e4 -->
* **Sentry CLI formatter dual-mode pattern: batch table vs streaming table**: List commands have two rendering paths: (1) \*\*Batch\*\*: \`formatXxxTable(items)\` — plain mode emits \`mdTableHeader()\` + \`mdRow()\`; TTY uses \`renderTextTable()\`. (2) \*\*Streaming\*\*: TTY uses \`StreamingTable\` with \`.header()/.row()/.footer()\`; plain uses raw markdown rows. Both share \`buildXxxRowCells()\` for consistent cell formatting. Dispatcher selects path based on \`asJson\`, \`table\` presence, or plain fallback.
...
* **Extract shared startSpinner helper to avoid poll/withProgress duplication**: Spinner logic in \`poll()\` and \`withProgress()\` is consolidated into \`startSpinner(stderr, initialMessage)\` returning \`{ setMessage, stop }\`. Key: \`stop()\` sets the stopped flag unconditionally (not guarded by \`json\` check). New spinner use cases should call \`startSpinner()\` rather than reimplementing the animation loop.
...
* **Sentry trace-logs API is org-scoped, not project-scoped**: The Sentry trace-logs endpoint (\`/organizations/{org}/trace-logs/\`) only requires org context, not project. It queries all projects in the org for logs associated with a trace ID. This differs from \`trace view\` which uses \`resolveOrgAndProject()\`. The \`trace logs\` command should use \`resolveOrg()\` instead. Params: \`traceId\`, \`statsPeriod\` (default 14d), \`per\_page\`, \`query\`, \`sort: -timestamp\`.
<!-- lore:019ca9c3-989c-7c8d-bcd0-9f308fd2c3d7 -->
* **Sentry CLI markdown-first formatting pipeline replaces ad-hoc ANSI**: Formatters build CommonMark strings; \`renderMarkdown()\` renders to ANSI for TTY or raw markdown for non-TTY. Key helpers: \`colorTag("red", text)\` for semantic colors, \`mdKvTable()\` for detail views, \`mdRow()\` for streaming rows, \`mdTableHeader()\` for headers (\`:\` suffix = right-aligned), \`renderTextTable()\` for batch tables with box-drawing. \`isPlainOutput()\` checks \`SENTRY\_PLAIN\_OUTPUT\` > \`NO\_COLOR\` > \`!isTTY\`. Two rendering paths: (1) Batch: \`formatXxxTable()\` — plain emits markdown, TTY uses \`renderTextTable()\`. (2) Streaming: TTY uses \`StreamingTable\`, plain uses raw markdown rows. Both share \`buildXxxRowCells()\` for consistent cell formatting.
<!-- lore:a1f33ceb-6116-4d29-b6d0-0dc9678e4341 -->
* **Issue list auto-pagination beyond API's 100-item cap**: Sentry API silently caps \`limit\` at 100 per request. \`listIssuesAllPages()\` auto-paginates using Link headers, bounded by MAX\_PAGINATION\_PAGES (50 pages). \`API\_MAX\_PER\_PAGE\` constant is shared across all paginated consumers. \`--limit\` means total results everywhere (max 1000, default 25). Org-all mode uses \`fetchOrgAllIssues()\` helper; explicit \`--cursor\` does single-page fetch to preserve cursor chain.
