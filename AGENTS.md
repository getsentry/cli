...
* **Testing Stricli command func() bodies via spyOn mocking**: To unit-test a Stricli command's \`func()\` body without HTTP calls: (1) \`const func = await viewCommand.loader()\` to get the function. (2) \`func.call(mockContext, flags, ...args)\` with a mock context containing \`stdout: { write: mock() }\`, \`stderr: { write: mock() }\`, \`cwd\`, \`setContext: mock()\`. (3) Use \`spyOn\` on namespace imports of \`api-client\`, \`resolve-target\`, \`browser\` etc. to mock dependencies. The \`loader()\` return type is a union (\`CommandModule | CommandFunction\`) so \`.call()\` shows LSP errors — these are false positives that pass \`tsc --noEmit\` and work at runtime. Follow the pattern in \`test/commands/project/view.func.test.ts\` and \`test/commands/log/view.func.test.ts\`. File naming convention: \`\*.func.test.ts\`.
