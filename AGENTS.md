...
* **Bun compiled binary sourcemap options and size impact**: Binary build is two-step: (1) esbuild bundles TS→minified JS + \`.map\` (not \`Bun.build()\` — identifier collision bug oven-sh/bun#14585 + empty sourcemap \`names\`). Config: \`platform: "node"\`, \`format: "esm"\`, \`external: \["bun:\*"]\`. \`mkdirSync("dist-bin")\` required. (2) \`injectDebugId()\` + \`uploadSourcemaps()\` (URL prefix \`~/$bunfs/root/\`), then \`Bun.build({ compile: true, minify: false })\` for native binary. Sourcemaps uploaded then deleted. Bun virtual path: \`/$bunfs/root/bin.js\`. \`binpunch\` removes unused ICU data. esbuild's 27k+ sourcemap names are critical for Sentry stack traces.
...
* **Nightly delta upgrade buildNightlyPatchGraph fetches ALL patch tags — O(N) HTTP calls**: Delta upgrade (\`src/lib/delta-upgrade.ts\`) supports stable (GitHub Releases) and nightly (GHCR) channels. \`filterAndSortChainTags\` filters \`patch-\*\` tags by version range via \`Bun.semver.order()\`. GHCR uses \`fetchWithRetry\` (10s timeout + 1 retry; blobs 30s) with \`signal?: AbortSignal\` combined via \`AbortSignal.any()\`. \`isExternalAbort(error, signal)\` skips retries for external aborts — critical for background prefetch. Patches cached to \`~/.sentry/patch-cache/\` (file-based, 7-day TTL). \`validateChainStep\` returns discriminated union \`ChainStepResult\` with typed failure reasons. CI tag filtering: \`grep '^nightly-\[0-9]'\` to exclude non-versioned tags (a stale \`nightly-test\` tag once poisoned all patches).
...
<!-- lore:019d738f-b7a1-7852-82d9-8c20b2503f89 -->
...
* **Sentry SDK tree-shaking patches must be regenerated via bun patch workflow**: The CLI uses \`patchedDependencies\` in \`package.json\` to tree-shake unused exports from \`@sentry/core\` and \`@sentry/node-core\` (AI integrations, feature flags, profiler, etc.). When bumping SDK versions: (1) remove old patches and \`patchedDependencies\` entries, (2) \`rm -rf ~/.bun/install/cache/@sentry\` to clear bun's cache (edits persist in cache otherwise), (3) \`bun install\` fresh, (4) \`bun patch @sentry/core\` then edit files and \`bun patch --commit\`, repeat for node-core. Key preserved exports: \`\_INTERNAL\_safeUnref\`, \`\_INTERNAL\_safeDateNow\` (core), \`nodeRuntimeMetricsIntegration\` (node-core). Manually generating patch files with \`git diff\` may fail — bun expects specific hash formats. Always use \`bun patch --commit\` to generate patches.

<!-- lore:019cb162-d3ad-7b05-ab4f-f87892d517a6 -->
* **Shared pagination infrastructure: buildPaginationContextKey and parseCursorFlag**: Schema v12 uses \`cursor\_stack TEXT\` (JSON array) + \`page\_index INTEGER\` in \`src/lib/db/pagination.ts\`. \`resolveCursor(flag, key, contextKey)\` maps keywords (next/prev/first/last) to \`{cursor, direction}\`. \`advancePaginationState\` pushes/pops stack — back-then-forward truncates stale entries. \`hasPreviousPage\` checks \`page\_index > 0\`. \`paginationHint()\` builds bidirectional hints (\`-c prev | -c next\`). JSON envelope includes \`hasPrev\`. All list commands use this stack API. \`resolveCursor()\` must be called inside \`org-all\` override closures, not before \`dispatchOrgScopedList\`.
