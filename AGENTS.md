...
* **Identity-scoped response cache via fingerprint mixin**: Identity-scoped response cache: key is \`hmac-sha256(identity, method|url)\` with module-constant HMAC key (quiets CodeQL weak-hash). Identity from \`getIdentityFingerprint()\` in \`db/auth.ts\` — must mirror \`getAuthConfig\` precedence exactly, including skipping expired access-only rows (no refresh\_token). Returns \`oauth:\<hash(refresh)>\`, \`oauth-access:\<hash(token)>\`, \`env:\<hash(token)>\`, or \`ANON\_IDENTITY\`. \`CacheEntry\` persists identity so \`invalidateCachedResponsesMatching(prefix)\` skips other identities' entries. All invalidation helpers (\`invalidateCachedResponse\`, \`invalidateCachedResponsesMatching\`, \`invalidateIssueCaches\`, \`invalidateIssueDetailCaches\`, \`invalidateOrgIssueList\`, \`invalidateOrgProjectCache\`) are contractually no-throw — wrap whole body in try/catch with swallow-and-log so post-mutation FS/DB errors can't mask successful API mutations (would cause retries/duplicates). Document with JSDoc: 'Best-effort: failures swallowed.' \`clearAuth()\` dynamically imports \`clearResponseCache\` to break db/auth ↔ response-cache cycle. Batch mutations: per-ID detail invalidation + single post-loop list sweep avoids N+1 directory scans.
