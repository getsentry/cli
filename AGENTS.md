...
* **Bun mock.module for node:tty requires default export and class stubs**: Bun testing gotchas: (1) \`mock.module()\` for CJS built-ins needs \`default\` re-export plus named exports. (2) Must be top-level BEFORE \`await import()\` — static imports capture bindings early. Lives in \`test/isolated/\` (runs via \`test:isolated\`). (3) Destructuring imports (\`import { stat } from 'node:fs/promises'\`) capture binding at load; verify via call-count > 0. (4) \`Bun.mmap()\` always opens PROT\_WRITE — use \`new Uint8Array(await Bun.file(path).arrayBuffer())\`. (5) Wrap \`Bun.which()\` with optional \`pathEnv\` for deterministic testing. (6) Mocking \`@sentry/node-core/light\`: \`startSpan\` must pass mock span to callback: \`startSpan: (\_, fn) => fn({ setStatus(){}, setAttribute(){}, end(){} })\`.

<!-- lore:019d870a-7baa-73f0-bf18-f815f427a8d4 -->
* **Sentry issue list --query passes OR/AND operators to API causing 400**: Sentry search does NOT support AND/OR (\`allow\_boolean=False\`). PEG grammar (\`script/search-query.pegjs\`) compiled to \`src/generated/search-parser.js\` via Peggy. \`sanitizeQuery()\` in \`src/lib/search-query.ts\` is the Stricli \`parse\` fn on all 7 \`--query\` flags (issue/trace/span/log list, trace logs, event list, issue events); dashboard widget add/edit excluded. AND stripped transparently; OR rewrites \`key:val1 OR key:val2\` → \`key:\[val1,val2]\`. Blocked for: \`is:\`/\`has:\`, negated, comparison, wildcards, free-text, different keys, paren groups. \`free\_text\` matches whole words. \`generate:parser\` runs before \`generate:sdk\`; generated files gitignored. Successful rewrites warn; failed throw \`ValidationError\`. Exports \`SEARCH\_SYNTAX\_REFERENCE\`.
