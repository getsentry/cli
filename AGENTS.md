...
* **Centralized search query sanitization in src/lib/search-query.ts**: \`sanitizeQuery()\` lives in \`src/lib/search-query.ts\`, not in any command file. All 7 commands with \`--query\` for Sentry search import and call it: \`issue list\`, \`trace list\`, \`span list\`, \`log list\`, \`trace logs\`, \`event list\`, \`issue events\`. The function uses its own \`logger.withTag("search-query")\` — callers don't pass a logger. Dashboard \`widget add/edit\` are excluded (their \`--query\` means aggregate expressions). The module also exports \`LIST\_QUERY\_FLAG\` constant and \`QUERY\_TOKEN\_RE\` regex for reuse. Pattern: sanitize early in \`func()\`, shadow \`flags\` with sanitized copy (\`const flags = { ...rawFlags, query: sanitizedQuery }\`) to avoid mutation issues with readonly flag objects.
