...
* **Sentry CLI command docs are auto-generated from Stricli route tree with CI freshness check**: Command docs in \`docs/src/content/docs/commands/\*.md\` and skill reference files in \`plugins/sentry-cli/skills/sentry-cli/references/\*.md\` are auto-generated from Stricli route tree introspection (\`src/lib/introspect.ts\`). Content between \`\<!-- GENERATED:START/END -->\` markers is regenerated; hand-written examples go below. Run \`bun run generate:command-docs\` and \`bun run generate:skill\` after changing command parameters/flags/docs. CI checks \`check:command-docs\` and \`check:skill\` fail if stale. The skill generator uses \`marked.lexer()\` AST parsing (not regex) to extract examples from doc pages — walking heading tokens for command paths and code tokens for bash blocks. \`check-skill.ts\` also verifies every reference file contains at least one \`\*\*Examples:\*\*\` section. Command index page order derives from \`extractAllRoutes()\` insertion order, not a manual constant.
