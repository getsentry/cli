...
* **Response cache hit invisibility — synthetic Response carries no marker**: Response cache hit invisibility — solved via module-level state: Cache hits in \`src/lib/response-cache.ts\` \`getCachedResponse()\` return a synthetic \`Response\` indistinguishable from network. Solved via module-level \`lastCacheHitAgeMs\`: set on hit, cleared at top of \`authenticatedFetch()\` per-call (single-process CLI = race-free). \`src/lib/cache-hint.ts\` provides \`formatCacheHint()\` (\`"cached · 3m ago · use -f to refresh"\`) and \`appendCacheHint(existingHint)\` (joins with \` | \`). Commands wire into \`return { hint }\` paths; for \`buildOrgListCommand\` factory consumers (team/repo list), update the factory's hint return. Skip bare \`return;\` paths (e.g. \`--web\`) since no API call was made. Other signals (Sentry span \`cache.hit\`, \`cache.hit\_rate\` metric, debug log) are not programmatically accessible from the Response itself.
