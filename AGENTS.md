...
* **Centralized search query sanitization in src/lib/search-query.ts**: \`sanitizeQuery()\` lives in \`src/lib/search-query.ts\`, not in any command file. All 7 commands with \`--query\` for Sentry search import and call it: \`issue list\`, \`trace list\`, \`span list\`, \`log list\`, \`trace logs\`, \`event list\`, \`issue events\`. The function accepts \`string | undefined\` via overloads — returns \`undefined\` when given \`undefined\`, \`string\` when given \`string\`. Callers simply write \`sanitizeQuery(flags.query)\` with no conditional guard. The module also exports \`LIST\_QUERY\_FLAG\` constant and \`QUERY\_TOKEN\_RE\` regex for reuse. Dashboard \`widget add/edit\` are excluded (their \`--query\` means aggregate expressions). Pattern for flags-shadow style: \`const flags = { ...rawFlags, query: sanitizeQuery(rawFlags.query) }\` or just \`const query = sanitizeQuery(flags.query)\`.
...
* **Sentry search in-list syntax constraints from PEG grammar**: Sentry's \`text\_in\_filter\` PEG grammar supports \`key:\[val1,val2]\` for any \`text\_key\` except \`is\` (explicitly documented as unsupported) and \`has\` (uses \`search\_value\` not \`text\_in\_list\`). Negated in-list \`!key:\[val1,val2]\` is grammatically valid but means "NOT in list" (exclude all matching), which differs from "exclude val1 OR exclude val2" — so OR-to-in-list rewrite must reject negated qualifiers. Wildcards are not supported inside in-list values. Quoted values work: \`key:\["val with spaces",val2]\`. Free-text tokens (no \`key:\` prefix) have no in-list equivalent — they're implicit CONTAINS searches. Grammar source: \`sentry/static/app/components/searchSyntax/grammar.pegjs\`.
...
* **Sentry issue list --query passes OR/AND operators to API causing 400**: Sentry issue search does NOT support AND/OR (\`allow\_boolean=False\`). \`sanitizeQuery()\` in \`src/lib/search-query.ts\` (centralized, used by all 7 \`--query\` commands) strips AND transparently (implicit AND semantics identical). For OR, attempts smart rewrite: \`key:val1 OR key:val2\` → \`key:\[val1,val2]\` in-list syntax. Supports merging existing brackets: \`key:\[a,b] OR key:c\` → \`key:\[a,b,c]\`. Quote-aware value splitting handles \`key:\["a,b",c]\`. Rewrite blocked for: \`is:\`/\`has:\` keys, negated qualifiers, wildcards, free-text tokens, different keys across OR boundary. Successful rewrites warn via \`logger.warn()\`. Failed rewrites throw \`ValidationError\`. Excluded: \`dashboard widget add/edit\` where \`--query\` means aggregate expressions. \`\_\_testing\` export on \`search-query.ts\` exposes internals for tests.
