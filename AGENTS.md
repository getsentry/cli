...
* **Identity-scoped response cache via fingerprint mixin**: Identity-scoped response cache: \`buildCacheKey(method, url)\` reads identity internally via memoized \`getIdentityFingerprint()\` — 2-arg signature. Uses SHA-256 of \`kind|secret\` truncated to 16 hex (MD5 rejected by CodeQL broken-crypto taint). \`ANON\_IDENTITY = "\<anon>"\`. \`CacheEntry\` persists identity so \`invalidateCachedResponsesMatching(prefix)\` skips other identities. All invalidation helpers are contractually no-throw. \`clearAuth()\` dynamically imports \`clearResponseCache\` to break db/auth ↔ response-cache cycle. Batch mutations: per-ID detail invalidation + single post-loop list sweep avoids N+1. \*\*Gotcha:\*\* exact-match \`invalidateCachedResponse()\` silently fails on URLs with query params (e.g. \`getIssue(id, {collapse:\[...]})\` caches as \`/issues/{id}/?collapse=...\`) — use \`invalidateCachedResponsesMatching(prefix)\` with trailing-slash URL as prefix for any endpoint called with \`params\`.
