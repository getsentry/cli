...
**Sentry CLI** is a command-line interface for [Sentry](https://sentry.io), built with [Stricli](https://bloomberg.github.io/stricli/) and bundled via esbuild + fossilize (Node SEA). Source code uses [Bun](https://bun.sh) APIs (shimmed for Node.js via `script/node-polyfills.ts`).
...
pnpm run cli -- --help                   # Dev with args
