...
* **Sentry search in-list syntax constraints from PEG grammar**: Sentry's \`text\_in\_filter\` PEG grammar supports \`key:\[val1,val2]\` for any \`text\_key\` except \`is\` (unsupported) and \`has\` (uses \`search\_value\` not \`text\_in\_list\`). In-list values must be plain text or quoted strings — comparison operators, wildcards, and negation are NOT valid inside brackets. Negated in-list \`!key:\[val1,val2]\` means "NOT in list". The CLI uses regex-based detection rather than the full PEG/Peggy parser. If Peggy is adopted: use pre-compiled approach (\`bun add -d peggy\`, \`generate:parser\` build script, commit generated \`.js\`). Sentry's full grammar (13.7KB, ~55 rules) is coupled to a ~700-line \`TokenConverter\` class with React deps — write a simplified CLI-specific grammar (~100-150 lines) instead. Generated parser cost: ~10KB minified (<1% of bundle). Signal to adopt: when new value types break in-list and can't be caught by prefix checks.
...
* **Sentry issue list --query passes OR/AND operators to API causing 400**: Sentry issue search does NOT support AND/OR (\`allow\_boolean=False\`). \`sanitizeQuery()\` in \`src/lib/search-query.ts\` strips AND transparently (implicit AND semantics identical). For OR, attempts smart rewrite: \`key:val1 OR key:val2\` → \`key:\[val1,val2]\` in-list syntax. Supports merging existing brackets: \`key:\[a,b] OR key:c\` → \`key:\[a,b,c]\`. Quote-aware value splitting handles \`key:\["a,b",c]\`. Rewrite blocked for: \`is:\`/\`has:\` keys, negated qualifiers, wildcards, free-text tokens, different keys across OR boundary, and \*\*comparison values\*\* starting with \`>\`, \`<\`, \`=\`, or \`!\` (e.g., \`age:>24h\`) — in-list syntax only accepts plain text values per the PEG grammar \[\[019d917d-cf52-7e98-b3db-b25979d445f2]]. Successful rewrites warn via \`logger.warn()\`. Failed rewrites throw \`ValidationError\`. \`COMPARISON\_VALUE\_RE\` in \`search-query.ts\` gates the check.
