...
<!-- lore:019d275c-7ced-74fd-8340-e11100194dbc -->
* **Stricli ExitCode.UnknownCommand is -5 but process.exitCode differs by runtime**: Stricli's \`ExitCode.UnknownCommand\` is \`-5\`. Bun converts negative exit codes to unsigned byte at storage time (\`process.exitCode\` reads \`251\`). Node.js preserves the raw value (\`process.exitCode\` reads \`-5\`). When checking for unknown command exit codes, compare against BOTH: \`process.exitCode === ExitCode.UnknownCommand || process.exitCode === (ExitCode.UnknownCommand + 256) % 256\`. The npm/Node distribution requires the dual check.

<!-- lore:019d275c-7cf0-7e13-bdc8-10cbbdbda933 -->
* **Stricli route errors are uninterceptable — only post-run detection works**: When Stricli's route scanner fails to match a subcommand (e.g., \`sentry issue helpp\`), it writes the error directly to \`context.process.stderr\` and returns \`ExitCode.UnknownCommand\` internally. No callback, hook, or exception is exposed. \`noCommandRegisteredForInput\` in \`customText\` only formats the message string — cannot inject side effects. The only interception point is checking \`process.exitCode\` after \`run()\` returns. The \`exceptionWhileRunningCommand\` callback only fires for errors thrown inside a command's \`func()\`, not for routing failures.
...
<!-- lore:019d275c-7cf4-76e1-9fe4-0d8e3b8a54cf -->
* **OutputError in handleOutputError calls process.exit() — prevents error propagation**: \`handleOutputError\` in \`buildCommand\`'s \`wrappedFunc\` calls \`process.exit(exitCode)\` for \`OutputError\`, which terminates the process immediately. This means: (1) Stricli's \`exceptionWhileRunningCommand\` callback never fires for \`OutputError\`. (2) \`withTelemetry\`'s catch block never captures these errors. (3) \`app.ts\`'s \`Sentry.captureException()\` in \`exceptionWhileRunningCommand\` never fires for help command's fuzzy-match errors. Top-level typos (\`sentry isseu\`) go through \`defaultCommand:help\` → \`OutputError\` → \`process.exit(1)\` — completely bypassing all error reporting.
...
* **Redact sensitive flags in raw argv before sending to telemetry**: When sending raw argv to Sentry telemetry (e.g., for unknown command reporting), redact values following sensitive flag names. The \`SENSITIVE\_FLAGS\` set in \`telemetry.ts\` lists flags to redact (currently just \`token\`). Pattern: scan argv for \`--token\` or \`-token\`, replace the following value with \`\[REDACTED]\`. Check both \`--flag value\` and \`--flag=value\` forms. The existing \`setFlagContext\` already handles this for parsed flags, but raw argv needs manual redaction.
...
<!-- lore:019d275c-7cf7-7b33-a121-906421ef64a0 -->
* **Telemetry reporting inside withTelemetry callback has full user/instance context**: \`withTelemetry\` calls \`initTelemetryContext()\` (sets \`Sentry.setUser()\` from SQLite, instance ID tag) BEFORE the \`startSpanManual\` callback runs. Any \`Sentry.captureMessage()\` or \`captureException()\` inside the callback automatically includes user ID, email, instance ID, runtime, and is\_self\_hosted tags. No extra context setup needed for reporting inside the callback. For org context, read \`getDefaultOrganization()\` from SQLite (no API call).
