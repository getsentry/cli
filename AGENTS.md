...
* **Sentry log IDs are UUIDv7 — enables deterministic retention checks**: Sentry log IDs are UUIDv7 (first 12 hex = ms timestamp, version char \`7\` at position 13). Traces and event IDs are NOT v7. \`decodeUuidV7Timestamp()\` and \`ageInDaysFromUuidV7()\` in \`src/lib/hex-id.ts\` return null for non-v7 IDs, safe to call unconditionally. Enables deterministic 'past 90-day log retention' messages instead of 'may have been deleted'. Wired in \`recoverHexId\` (past-retention short-circuit) and \`log/view.ts#throwNotFoundError\`. \`RETENTION\_DAYS.log = 90\` in \`src/lib/retention.ts\`; traces/events are \`null\` (plan-dependent).
...
* **Bugbot flags defensive null-checks as dead code — keep them with JSDoc justification**: Bugbot/Seer false positives to dismiss with JSDoc: (1) defensive null-checks flagged as dead code. (2) stderr spinner during \`--json\` — progress is stderr, JSON is stdout. (3) \`Number(projectData.id)\` without \`isFinite\` — Sentry project IDs always numeric strings. Real bugs bots catch: \`mock.module()\` in non-isolated test files, missing lower-bound assertions on tracked values. PR review workflow: separate fixup commit per round (not amend), reply via \`gh api\` REST, resolve threads via GraphQL \`resolveReviewThread\`.

<!-- lore:019cc3e6-0cdd-7a53-9eb7-a284a3b4eb78 -->
* **Bun mock.module for node:tty requires default export and class stubs**: Bun testing gotchas: (1) \`mock.module()\` for CJS built-ins needs \`default\` re-export plus named exports, top-level BEFORE \`await import()\` — static imports capture bindings early. Lives in \`test/isolated/\`. (2) Destructuring imports capture binding at load; verify via call-count > 0. (3) \`Bun.mmap()\` always opens PROT\_WRITE — use \`new Uint8Array(await Bun.file(path).arrayBuffer())\`. (4) Wrap \`Bun.which()\` with optional \`pathEnv\` for deterministic testing. (5) Mocking \`@sentry/node-core/light\`: \`startSpan\` must pass mock span to callback: \`startSpan: (\_, fn) => fn({ setStatus(){}, setAttribute(){}, end(){} })\`.

<!-- lore:019d870a-7baa-73f0-bf18-f815f427a8d4 -->
* **Sentry issue list --query passes OR/AND operators to API causing 400**: Hex ID recovery decision tree in \`src/lib/hex-id-recovery.ts\` heals malformed IDs across \`event/trace/log/span view\`: (1) \`preNormalize\` strips URL fragments, UUID dashes, sentinel leaks. (2) \`stripTrailingNonHex\` handles \`<32hex>ios\`. (3) UUIDv7 retention check short-circuits expired log IDs to \`past-retention\`. (4) \`extractHexCandidate\` handles hex prefixes and middle-ellipsis (\`abc...def\`). (5) \`looksLikeSlug\` detects slug-as-ID with per-entity redirects in \`SLUG\_REDIRECT\_COMMAND\`. (6) Class E cross-entity: 16-hex to \`trace view\` resolves parent via \`findTraceBySpanId\`. Adapter signature \`(ctx) => IDs\[]\`; caller filters via \`filterByCandidate\`. \`MIN\_FUZZY\_PREFIX=8\` matches Sentry UI's \`getShortEventId\`. \`handleRecoveryResult\` emits \`log.warn\` on success, \`ValidationError\`/\`ResolutionError\` on failure. Validation deferred until after target resolution so adapters have org/project context.
...
* **Pagination contextKey must include all query-varying parameters with escaping**: Pagination \`contextKey\` must encode every query-varying parameter (sort, query, period) with \`escapeContextKeyValue()\` (replaces \`|\` with \`%7C\`). Provide fallback before escaping since \`flags.period\` may be \`undefined\` in tests despite defaults: \`flags.period ? escapeContextKeyValue(flags.period) : "90d"\`. \`DEFAULT\_PERIOD = "90d"\` is a module constant in commands that need it. Compare periods via \`formatTimeRangeFlag(flags.period) !== DEFAULT\_PERIOD\` or \`appendPeriodHint()\` from \`time-range.ts\`.
...
<!-- lore:019da557-63e0-7d38-92b7-a0a415578ccb -->
* **Shared hex regexes + retention constants live in dedicated modules**: Shared primitives live in dedicated modules: hex regex primitives (\`HEX\_ID\_RE\`, \`SPAN\_ID\_RE\`, \`UUID\_DASH\_RE\`, \`LEADING\_HEX\_RE\`, \`MIDDLE\_ELLIPSIS\_RE\`, etc.) in \`src/lib/hex-id.ts\` — never duplicate in consumers like \`hex-id-recovery.ts\`. Retention constants (\`RETENTION\_DAYS\`, \`SCAN\_PERIODS\`) in \`src/lib/retention.ts\` — shared between \`api/logs.ts\` (\`statsPeriod\` defaults) and \`hex-id-recovery.ts\` (fuzzy-scan windows). Keep API-facing UI defaults (\`'7d'\` list commands) separate from recovery scan windows (event 90d, trace/log/span 30d).

<!-- lore:019d91ea-bb02-78d4-bf11-743b99f0ba6b -->
* **Stricli parse functions can perform validation and sanitization at flag-parse time**: Stricli's \`parse\` fn on \`kind: "parsed"\` flags runs during argument parsing before \`func()\`. Can throw errors (including \`ValidationError\`) and log warnings. Current uses: \`parseCursorFlag\`, \`sanitizeQuery\`, \`parsePeriod\` (returns \`TimeRange\`), \`parseSort\`/\`parseSortFlag\`, \`numberParser\`/\`parseLimit\`. Optional period flags: \`flags.period\` is \`TimeRange | undefined\` — commands use \`TIME\_RANGE\_\*\` constants as defaults. \`formatTimeRangeFlag()\` converts back to display strings. \`appendPeriodHint(parts, period, flagName)\` in \`time-range.ts\` encapsulates the \`if (formatted !== DEFAULT\_PERIOD) parts.push(...)\` pattern used across 4+ hint builders.
