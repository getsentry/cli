...
<!-- lore:019d484d-9364-7d8e-b95b-1dd413bc4054 -->
* **Craft artifact discovery uses commit SHA and workflow name, not tags**: Craft artifact discovery and release pipeline: Craft (\`getsentry/craft@v2\`) finds artifacts by commit SHA + workflow \*\*name\*\* (not tags). Jobs in \`ci.yml\` (named \`Build\`) auto-discovered; artifacts matching \`/^sentry-.\*$/\` picked up without \`.craft.yml\` changes. GitHub Releases are immutable post-publish — all assets (including delta patches) must be workflow artifacts before craft runs. Delta patches use TRDIFF10 (zig-bsdiff + zstd). \`generate-patches\` job runs on \`main\`/\`release/\*\*\` with \`continue-on-error\`. Nightly: old binary from GHCR via ORAS. Stable: \`gh release download\`. Binary download steps use \`sentry-\*-\*\` pattern to avoid matching \`sentry-patches\`. Bash glob loops need \`shopt -s nullglob\`.

<!-- lore:019da076-2cd3-716c-82ff-1779d8f4cab7 -->
* **Fuzzy recovery auto-resolves dash/underscore slug mismatches without original-slug fallback**: The three project-search resolution sites (\`resolveProjectBySlug\`, \`resolveOrgProjectTarget\`, \`handleProjectSearch\`) plus \`project/list.ts\`'s own \`handleProjectSearch\` (4 total) all call \`tryFuzzyProjectRecovery\` when \`findProjectsBySlug\` returns empty. The fuzzy threshold \`max(2, floor(len/3))\` means single-char substitutions (dash↔underscore, space↔dash) are always within range. When exactly one match is found, all four sites auto-recover with \`log.warn\`. \`originalSlug\` was added to \`ParsedOrgProject\`'s \`project-search\` variant (#774) so warning messages reference the user's actual input (e.g., "My Project") instead of the normalized slug ("my-project"). Issue #772 was reopened with this revised scope — the retry-with-original-slug approach was unnecessary.

<!-- lore:019da04c-1a08-75b9-a752-fd32ac2db73a -->
* **Project cache is org-scoped with three key formats and three population paths**: The \`project\_cache\` SQLite table stores org/project metadata with three cache key formats: \`"{orgId}:{projectId}"\` (DSN resolution), \`"dsn:{publicKey}"\` (DSN without orgId), \`"list:{orgSlug}/{projectSlug}"\` (batch from API). Key functions: \`getCachedProjectsForOrg(orgSlug)\` returns all cached projects for one org (no cross-org search exists). Population paths: (1) DSN resolution in resolve-target.ts writes single entries, (2) \`listProjects()\` in api/projects.ts batch-writes via \`cacheProjectsForOrg\` (best-effort, try-catch guarded), (3) JSON config migration (one-time). Fuzzy search against cache happens only in \`completeProjectSlugs()\` for shell completions — resolution errors use live API calls via \`findSimilarProjectsAcrossOrgs\` instead.
...
<!-- lore:019da04c-1a0c-76c8-b608-e7da0e3db063 -->
* **Three resolution paths for project-search slugs each have independent error/fuzzy logic**: When \`ParsedOrgProject\` has \`type: "project-search"\`, \*\*four\*\* independent resolution paths handle it: (1) \`resolveProjectBySlug\` (resolve-target.ts) for view commands. (2) \`resolveOrgProjectTarget\` (resolve-target.ts) for trace/log commands. (3) \`handleProjectSearch\` (org-list.ts) for generic list commands. (4) \`handleProjectSearch\` (project/list.ts) for project-specific list. All four have independent fuzzy-recovery and error formatting. The \`originalSlug\` field (added in #774) must be threaded to all four sites — \`resolveProjectBySlug\` takes it as an optional 4th parameter, \`resolveOrgProjectTarget\` reads \`parsed.originalSlug\`, and both \`handleProjectSearch\` variants accept it in their options/parameters. Any cross-cutting slug resolution change must touch all four sites.
...
<!-- lore:019da03d-8c36-7b48-b366-b9d9530ed405 -->
* **Property test slug generators must not assume underscore-free slugs**: After removing underscore normalization from \`normalizeSlug()\`, property test generators for slugs (\`slugLikeWithUnderscoresArb\`, \`displayNameLikeArb\`) needed updating. The old invariant "result slug never contains underscores" was wrong — underscores pass through unchanged now. Only the space-related invariants remain: "result slug never contains spaces", "normalized iff input contained spaces", "lowercase when spaces present". The \`displayNameLikeArb\` regex \`\[A-Za-z]\[A-Za-z0-9 \_-]\` still generates underscores but they're no longer treated as normalization triggers.
...
<!-- lore:019da03d-8c2b-74ca-933d-27e6af20e9ea -->
* **Sentry allows underscores in project/org slugs — CLI must not normalize them**: Sentry's API accepts underscores in project and org slugs. \`normalizeSlug()\` only normalizes spaces→dashes (+ lowercase) — underscores pass through unchanged. The \`originalSlug\` field was added to \`ParsedOrgProject\`'s \`project-search\` variant (#774) to preserve the user's pre-normalization input for error messages. When normalization fires and fuzzy recovery kicks in, warnings now reference the original input (e.g., "My Project") instead of the normalized slug ("my-project"). The retry-with-original-slug approach was confirmed unnecessary — fuzzy recovery handles dash↔underscore and space↔dash mismatches within the Levenshtein threshold \`max(2, floor(len/3))\`.
