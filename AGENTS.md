...
* **Fuzzy recovery auto-resolves dash/underscore slug mismatches without original-slug fallback**: The three project-search resolution sites (\`resolveProjectBySlug\`, \`resolveOrgProjectTarget\`, \`handleProjectSearch\`) all call \`tryFuzzyProjectRecovery\` when \`findProjectsBySlug\` returns empty. The fuzzy threshold formula \`max(2, floor(len/3))\` means single-character substitutions (dash↔underscore) are always within range. When exactly one fuzzy match is found, all three sites auto-recover transparently with a \`log.warn\`. This makes \`originalSlug\` plumbing through \`ParsedOrgProject\` unnecessary — verified empirically: \`levenshtein("my-project", "my\_project") = 1\`, threshold for 10-char slug = 3. Issue #772 closed as won't-fix.
...
* **Three resolution paths for project-search slugs each have independent error/fuzzy logic**: When \`ParsedOrgProject\` has \`type: "project-search"\`, three independent resolution paths handle it: (1) \`resolveProjectBySlug\` (resolve-target.ts:1259) for view commands — calls \`findProjectsBySlug\`, then \`tryFuzzyProjectRecovery\` for auto-recovery (single match) or suggestions (multiple). (2) \`resolveOrgProjectTarget\` (resolve-target.ts:1452) for trace/log commands — same fuzzy pattern. (3) \`handleProjectSearch\` (org-list.ts:705) for list commands — uses \`tryFuzzyRecoveryForList\` wrapper, recursively re-enters with corrected slug on single match. All three throw \`ResolutionError\` with "Similar projects:" suggestions from \`findSimilarProjectsAcrossOrgs\` which does live API calls (not cache). Any cross-cutting improvement to slug resolution must touch all three sites.
...
* **Sentry allows underscores in project/org slugs — CLI must not normalize them**: Sentry's API accepts underscores in project and org slugs. \`normalizeSlug()\` only normalizes spaces→dashes (+ lowercase) — underscores pass through unchanged. The original pre-normalization input is \*\*discarded\*\* at \`parseOrgProjectArg\` — only the normalized slug and a boolean flag survive into \`ParsedOrgProject\`. However, adding \`originalSlug\` fallback retry is \*\*unnecessary\*\*: the existing fuzzy recovery (\`tryFuzzyProjectRecovery\` → \`fuzzyMatch\`) already handles dash-vs-underscore mismatches. For a 10-char slug like \`my-project\` vs \`my\_project\`, Levenshtein distance is 1, threshold is 3 — single-match auto-recovery triggers at all three resolution sites with a \`log.warn\`. Issue #772 was closed as won't-fix for this reason.
