...
* **Response cache hit invisibility — synthetic Response carries no marker**: Response cache hit detection — synthetic \`Response\` from \`getCachedResponse()\` in \`src/lib/response-cache.ts\` is indistinguishable from network. Solved via module-level \`lastCacheHitAgeMs\`: set on hit, cleared at top of \`authenticatedFetch()\` per-call (single-process CLI = race-free). \`src/lib/cache-hint.ts\` provides \`formatCacheHint()\` (\`"cached · 3m ago · use -f to refresh"\`) and \`appendCacheHint(existingHint)\` (joins with \` | \`). \*\*Wiring centralized in \`buildCommand\`\*\* (\`src/lib/command.ts\`): calls \`appendCacheHint(returned?.hint)\` once at the \`returned?.hint\` chokepoint before \`writeFinalization\` — covers ALL commands automatically. Same chokepoint can host future cross-cutting hint decorators (deprecation warnings, telemetry footers) to avoid N-file diffs. Skip bare \`return;\` paths (e.g. \`--web\`) since no API call was made. Sentry span \`cache.hit\` and \`cache.hit\_rate\` metric are not programmatically accessible from the Response itself.
