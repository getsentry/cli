...
* **bunfig.toml \`timeout\` key is not supported by Bun — use \`--timeout\` CLI flag**: Bun 1.3.x does not support \`timeout\` as a \`bunfig.toml\` config key under \`[test]\`. Setting it there has no effect — all tests silently use the default 5000ms per-test timeout. The only ways to set test timeout are: (1) \`--timeout\` CLI flag on \`bun test\`, (2) third argument to \`test()\`, or (3) \`setDefaultTimeout()\` from \`bun:test\`. The project uses \`--timeout 15000\` in \`package.json\` test scripts. Model-based tests (50 iterations × full SQLite schema init × 10–30 command sequences) routinely need 5–13s per test.
