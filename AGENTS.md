...
* **Craft artifact discovery uses commit SHA and workflow name, not tags**: Craft (\`getsentry/craft@v2\`) discovers artifacts by commit SHA + workflow \*\*name\*\* (not tags). Jobs in \`ci.yml\` (named \`Build\`) auto-discovered; artifacts matching \`/^sentry-.\*$/\` picked up without \`.craft.yml\` changes. GitHub Releases are immutable post-publish — all assets (including delta patches) must be workflow artifacts before craft runs. Delta patches use TRDIFF10 (zig-bsdiff + zstd). \`generate-patches\` job runs on \`main\`/\`release/\*\*\` with \`continue-on-error\`. Nightly: old binary from GHCR via ORAS. Stable: \`gh release download\`. Binary download steps must use \`sentry-\*-\*\` pattern to avoid matching \`sentry-patches\`. Bash glob loops need \`shopt -s nullglob\`.

<!-- lore:019da076-2cd3-716c-82ff-1779d8f4cab7 -->
* **Fuzzy recovery auto-resolves dash/underscore slug mismatches without original-slug fallback**: Fuzzy recovery for project-search slugs threads originalSlug through 4 sites: \`normalizeSlug()\` lossy first guess can confuse users when real slug differs. \`ParsedOrgProject\`'s \`project-search\` variant carries \`originalSlug?: string\` (pre-normalization input). All four resolution sites (\`resolveProjectBySlug\`, \`resolveOrgProjectTarget\`, \`org-list.ts#handleProjectSearch\`, \`project/list.ts#handleProjectSearch\`) thread it into \`tryFuzzyProjectRecovery\` as \`displayName\`. When present, \`findSimilarProjectsAcrossOrgs\` fuzzy-matches against both slugs AND display names (enables \`"My Project"\` → \`my\_project\`). Fuzzy threshold \`max(2, floor(len/3))\` catches dash↔underscore and space↔dash mismatches. Pass \`originalSlug\` directly — falling back to slug triggers wasteful name matching. \`project/list.ts#handleProjectSearch\` needs its own \`isRecoveryAttempt\` guard (bundled into options with \`originalSlug\` to stay under Biome \`useMaxParams\` limit of 4) to prevent infinite recursion when recovered slug also fails. Any cross-cutting slug resolution change must touch all four sites.
...
* **Property test slug generators must not assume underscore-free slugs**: \`normalizeSlug()\` only normalizes spaces→dashes + lowercase — underscores pass through unchanged (Sentry API accepts them). Property test generators (\`slugLikeWithUnderscoresArb\`, \`displayNameLikeArb\`) must not assume underscore-free output. Valid invariants: result never contains spaces; normalized iff input contained spaces; lowercase when spaces present. Also avoid generators producing patterns like \`0x\[0-9a-f]+\` (hex addresses) or \`/home/\` prefixes that trigger unrelated normalization elsewhere.
...
* **PR workflow: address review comments, resolve threads, wait for CI**: PR workflow: (1) Wait for CI, (2) Check unresolved review comments via \`gh api\`, (3) Fix in follow-up commits (not amends), (4) Reply explaining fix, (5) Resolve thread via \`gh api graphql\` \`resolveReviewThread\` mutation, (6) Push and re-check CI, (7) Final sweep. Use \`git notes add\` for plans. Branch naming: \`fix/\*\` or \`feat/\*\`. Code style: \`Array.from(map.keys())\` over spread; "allowlist" not "whitelist"; reviewer questions may be inquiries, not change requests — confirm intent before changing.
