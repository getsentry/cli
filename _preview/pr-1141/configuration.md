...
url: "https://cli.sentry.dev/_preview/pr-1141/configuration/"
---
...
The CLI also detects DSNs from `.env` files and source code automatically — see [DSN Auto-Detection](https://cli.sentry.dev/_preview/pr-1141/configuration/features.md#dsn-auto-detection).
...
Base URL of your Sentry instance. **Only needed for [self-hosted Sentry](https://cli.sentry.dev/_preview/pr-1141/configuration/self-hosted.md).** SaaS users (sentry.io) should not set this.
...
Client ID of a public OAuth application on your Sentry instance. **Required for [self-hosted Sentry](https://cli.sentry.dev/_preview/pr-1141/configuration/self-hosted.md)** (26.1.0+) to use `sentry auth login` with the device flow. See the [Self-Hosted guide](https://cli.sentry.dev/_preview/pr-1141/configuration/self-hosted.md#1-create-a-public-oauth-application) for how to create one.
...
Custom HTTP headers to include in all requests to your Sentry instance. **Only applies to [self-hosted Sentry](https://cli.sentry.dev/_preview/pr-1141/configuration/self-hosted.md).** Ignored when targeting sentry.io.
...
Pin a specific version for the [install script](https://cli.sentry.dev/_preview/pr-1141/configuration/getting-started.md#install-script). Accepts a version number (e.g., `0.19.0`) or `nightly`. The `--version` flag takes precedence if both are set.
...
Force the output format for all commands. Currently only `json` is supported. This is primarily used by the [library API](https://cli.sentry.dev/_preview/pr-1141/configuration/library-usage.md) (`createSentrySDK()`) to get JSON output without passing `--json` flags.
...
See [`sentry cli defaults`](https://cli.sentry.dev/_preview/pr-1141/configuration/commands/cli.md#sentry-cli-defaults) for full usage.
...
See [Credential Storage](https://cli.sentry.dev/_preview/pr-1141/configuration/commands/auth.md#credential-storage) in the auth command docs for more details.
