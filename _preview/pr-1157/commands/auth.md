...
url: "https://cli.sentry.dev/_preview/pr-1157/commands/auth/"
---
...
[Section titled “Read-only login”](#read-only-login)
Terminal window

```
# Request only read scopes (project:read, org:read, event:read, etc.)sentry auth login --read-only
```
...
```
# Request specific OAuth scopes (comma-separated or repeated)sentry auth login --scope project:read --scope org:readsentry auth login --scope project:read,org:read,event:read
```
...
See [Self-Hosted Sentry](https://cli.sentry.dev/_preview/pr-1157/commands/self-hosted.md) for details.
